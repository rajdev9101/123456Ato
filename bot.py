...
async def offilter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await send_and_auto_delete(update, context, text="🧠 Use: /offilter hi = hello OR /offilter remove hi")
        return
    joined = " ".join(context.args)
    if joined.lower().startswith("remove"):
        key = joined[6:].strip().lower()
        if key in replies:
            del replies[key]
            save_json(REPLIES_FILE, replies)
            await send_and_auto_delete(update, context, text=f"🗑 Removed reply for: {key}")
        else:
            await send_and_auto_delete(update, context, text=f"⚠️ No reply found for: {key}")
    else:
        if "=" in joined:
            key, val = map(str.strip, joined.split("=", 1))
            if key and val:
                replies[key.lower()] = val
                save_json(REPLIES_FILE, replies)
